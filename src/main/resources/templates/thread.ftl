...
<head>
    ...
    <link href='https://fonts.googleapis.com/css?family=Roboto+Slab:400,300,700|Open+Sans+Condensed:300|Teko|Khand' rel='stylesheet'
    type='text/css'>
...
<main id="content" class="container">
    ...
    <h1>${thread["title"]}</h1>
    <h3>von ${thread["questionerDisplayName"]}</h3>
    <hr>
    <p>
        ${thread["body"]}
    </p>
    ...
    <h3>Antworten</h3>
    <#if thread.answers?size gt 0>
    <#list thread.answers as answer>

    <p><span class="answer-title">${answer["title"]}</span>
        <span class="answerer">von ${answer["answererDisplayName"]}</span>
    </p>
    <p>${answer["body"]}</p>
    <hr>
    ...
    <#if malformed??>
        <#if malformed == true>
            <p class="error">Antwort konnte leider nicht hinzugefügt werden.</p><br>
        </#if>
    </#if>

    <h4>Antwort hinzufügen</h4>
    <form action="${thread["id"]}/answer" method="post">
        Titel<br>
        <input type="text" name="title" id="title-input"><br>
        Antwort eingeben<br>
        <textarea name="body" id="answer-textarea" cols="30" rows="10">
        </textarea><br>
